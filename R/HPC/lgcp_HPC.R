...
multipliers <- c(0.1, 0.3, 0.5, 0.7)
sigma_list <- c(0.1, 0.5, 1)
smoother_list = c('tp', 'ps', 'cr')
n_knots_list = c(3, 4)

mult_combs <- crossing(max_edges, multipliers, smoother_list, n_knots_list, species_choices, sigma_list) # 432
comb_values <- mult_combs[job, ]
print(comb_values)

max.edge = comb_values$max_edges
range_multiplier = comb_values$multipliers
...
# save(model, all_preds, suitability, random_field, comb_values, mesh, fixed_effects_effort, proj_grid, file = paste0("model_out/lgcp_rufa_V6/", logCPO, "_all", fine_res, "m_", n_knots, "k_", smoother, "_", sub(" ", "_", species_choice), "_E", max.edge, "_mult", range_multiplier,"_r",
#           prior_range[1], "_", prior_range[2], "_s",
#           prior_sigma[1], "_", prior_sigma[2], ".RData"))

pdf(paste0("figures/", gsub(" ", "_", species_choice), "/lgcp/", 
