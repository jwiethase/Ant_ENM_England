...
dir.create(paste0("model_out/", gsub(" ", "_", species_choice), "/maxent_V2"), showWarnings = F)
dir.create(paste0("figures/", gsub(" ", "_", species_choice)), showWarnings = F)
dir.create(paste0("figures/", gsub(" ", "_", species_choice), "/maxent_V2"), showWarnings = F)
...
predictors <- raster::subset(raster::stack(covariates_stack), 
                             subset = c(names(covariates_stack)[!names(covariates_stack) %in% c("lat_raster", "distance_ancient", "forest_mask_buff", "forest_mask")]))  
...
# Get response curves
pdf(paste0("figures/", gsub(" ", "_", species_choice), "/maxent_V2/V2_effectPlots_", 
...
writeRaster(suitability_raster, 
            filename = paste0("model_out/", gsub(" ", "_", species_choice), "/maxent_V2/V2_sporadic_", 
                              gsub(" ", "_", species_choice), "_all30m_thin", thin_dist, "m.tif"), overwrite = T)

pdf(paste0("figures/", gsub(" ", "_", species_choice), "/maxent_V2/V2_sporadic_",
...
pdf(paste0("figures/", gsub(" ", "_", species_choice), "/maxent_V2/V2_sporadic_Unseen_", 
